...
# import camera_info_manager # UPDATED
import datetime
import os
...
class StreamThread(threading.Thread):
    ...
    def get_image_custom(self):
        '''Get image from directory instead from streaming'''
        img_np = cv2.imread(self.axis.fake_img_stream[self.axis.img_idx])
        self.img = np.array(cv2.imencode('.jpg', img_np)[1]).tostring()
        self.axis.img_idx += 1
...
class Axis:
    def __init__(self, hostname, username, password, width, height, fps, frame_id,
        ...
        if self.use_encrypted_password:
            # self.http_auth = requests.auth.HTTPDigestAuth(self.username, self.password) #UPDATE
            self.http_auth = None # ADDED            
        else:
        ...
        # self.cname = camera_info_manager.genCameraName(self.hostname)          # UPDATED
        # self.cinfo = camera_info_manager.CameraInfoManager(cname = self.cname, # UPDATED
